...
Анализирует целевой WAV-файл, оптимизирует конфигурацию осцилляторов (чистый coordinate descent) и создаёт JSON параметров осцилляторов + воссозданный WAV + визуализацию сравнения. Оптимизация выполняется в отдельном worker-потоке (`optimizer-worker.ts`), чтобы не блокировать основной поток.
