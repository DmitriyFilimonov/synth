...
> 2. **Спектральный член** $S_{\text{spec}}$ (30% веса) измеряет совпадение магнитуд на 5 доминантных частотах и может быть низким при хорошем временно́м совпадении.
...
| Оконечная оценка          | `evaluateSuppressionWindowed` — средний score по 100ms окнам при фиксированном масштабе 1 + спектральный scoring (Goertzel, кэшированный профиль таргета) + shape penalty |
...
| `src/optimize/`                     | Модуль оптимизации: `index.ts` (реэкспорт), `coordinate-descent.ts` (алгоритм), `evaluate.ts` (оценка, `WaveformCache`), `consts.ts` (константы), `types.ts` (типы), `staged.ts` (поэтапная или плоская оптимизация с HPO), `residual-relocation.ts` (relocation слабых осцилляторов на пики остатка) |
