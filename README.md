...
- **Огибающая амплитуды** $A_k(t) = \exp\bigl(\ln(e_k)\cdot (t/d_k)^{p_k}\bigr)$ — экспоненциальный спад от $1$ (при $t=0$) до `endLevel` $e_k$ (при $t=d_k$); при $t > d_k$ значение замораживается. Кривизна $p_k \in [0.2, 2]$.
...
> ⚠️ Две особенности модели, важные для анализа (подробно — в секции про 50%):
>
...
> 2. `ampEnv.startLevel` **не участвует в синтезе** (см. `envelopeCreator` в `src/envelope.ts`: параметр `max` игнорируется). Все активные осцилляторы стартуют с амплитуды 1.0.
...
| 9      | `startLevel`       | 0.001–1       | Трактуется оптимизатором как «громкость», но **не влияет на сигнал** |
...
### 2. Квантование частоты линейной нормализацией — жёсткий потолок точности

Частота кодируется линейно: $[0,1] \mapsto [20, 20000]$ Гц. Минимальный шаг спуска в фазе PRECISION — $10^{-4}$, что соответствует **2 Гц**. Из таблицы выше: ошибка 2 Гц на полном сигнале даёт _отрицательное_ подавление. Чтобы выйти на 98%, нужна точность ~0.01 Гц, т.е. шаг $5\cdot10^{-7}$ — на **три порядка** мельче минимального. При этом такой же шаг $10^{-4}$ для фазы — это 0.036°, т.е. сетка по фазе избыточно мелкая, а по частоте — катастрофически грубая. Единая шкала шага для разнородных параметров не работает.
...
### 4. Баг: параметр «громкости» не влияет на сигнал

`envelopeCreator` (`src/envelope.ts`) игнорирует поле `max`, в которое синтезатор передаёт `ampEnv.startLevel`. Проверено прямым экспериментом: сигналы с `startLevel = 1.0` и `= 0.05` **побитово идентичны**. Следствия:

- у модели **нет индивидуальной громкости осциллятора** — каждый активный партиал стартует с амплитуды 1.0. Реальные звуки имеют спад амплитуд гармоник (часто ~1/n); модель принципиально не может это представить, кроме как через манипуляции `endLevel`/`duration`;
- оптимизатор при этом честно тратит ходы на offset 9 (2 полных ресинтеза на осциллятор за итерацию) — эти оценки всегда бесполезны;
- **финальный scale fitting — no-op**: `findOptimalScale` находит лучший множитель по _отмасштабированному waveform_, записывает score, а потом умножает мёртвый параметр генома. Отчётный score может быть недостижим для возвращённого генома;
- прунинг и `enforceFlagInvariant` принимают решения по значению мёртвого параметра.

### 5. Жёсткий клиппинг суммы

`createSynth` клампит сумму в $[-1, 1]$. Поскольку все $N$ активных осцилляторов стартуют с амплитуды 1.0 (см. п. 4), при $N \gg 1$ сигнал в начале почти гарантированно клиппится — сильная невыпуклая нелинейность поверх и без того сложного ландшафта. Джобы с 30–45 осцилляторами работают в режиме перманентного клиппинга.
...
- Per-window свободный скаляр громкости внутри $J$ разрешает то, что геном воспроизвести не может (громкость фиксирована и мертва) — оптимизатор «улучшает» $J$ без улучшения реального подавления;
...
1. **Починить громкость** (п. 4): использовать `max` в `envelopeCreator` как стартовый уровень (или ввести явный per-osc gain). Это одновременно чинит scale fitting, прунинг и клиппинг (п. 5). Без этого остальное почти бессмысленно.
2. **Сменить параметризацию частоты** (п. 2): лог-шкала (20 Гц–20 кГц ≈ 10 октав; полутоновая/центовая сетка естественна и для музыкального слуха) + отдельные масштабы шага на класс параметра.
...
5. **Согласовать объектив** (п. 7): убрать per-window скейл, когда появится настоящая громкость; добавить wrap-around фазы; кешировать спектральный анализ таргета (п. 8 — бесплатное ускорение на порядок).
...
7. Если оставаться в derivative-free парадигме — CMA-ES или Powell вместо покоординатного спуска (устойчивы к гребням), но пп. 1–4 дают больше при меньших затратах.

> Графики в `docs/img/` сгенерированы аналитически по формулам из этой секции; числовые утверждения о поведении кода (мёртвый `startLevel`, реальная мгновенная частота) проверены прямыми экспериментами на модулях `src/synth.ts` / `src/envelope.ts`.
...
> Обратите внимание: параметр `max` (`ampEnv.startLevel`) в формуле отсутствует — `envelopeCreator` его игнорирует, огибающая всегда стартует с 1.0. Это ключевой дефект для качества подбора, см. [«Баг: параметр громкости не влияет на сигнал»](#4-баг-параметр-громкости-не-влияет-на-сигнал).
