...
Эта секция описывает задачу на трёх языках — саунд-дизайнера, математика и ML-инженера — и содержит всё необходимое, чтобы понять, **почему рост метрики подавления замедляется и платонирует** (см. [анализ ниже](#почему-подавление-растёт-медленно--анализ)).
...
> ⚠️ Особенность модели, важная для анализа (подробно — в секции анализа ландшафта метрики ниже):
>
...
**Целевой уровень:** `suppression ≥ 98%` (совпадает с ранним выходом
...
Однако внутри coordinate descent оптимизируется не она, а **композитный multi-scale windowed surrogate** (`evaluateMultiScaleWindowed`, `src/optimize/evaluate.ts`):
...
> ⚠️ **J занижен относительно честного глобального suppression** (формула выше, `1 − RMS(остаток)/RMS(таргет)` — то, что слышно при вычитании результата из таргета в DAW). Три фактора суммируются:
>
...
>
> Практический вывод: субъективное качество результата (прослушивание / вычитание в DAW) **систематически лучше**, чем показывает число в job-статусе. Поэтому в результатах job и sync-ответах API возвращается дополнительное поле **`globalSuppressionPercent`** — честный глобальный suppression финального WAV (`assessCancellationQuality` по полному сигналу; `null`, если оценка не удалась). Оценивайте результат по WAV и по этому полю, а не только по $J$.
...
## Почему подавление растёт медленно — анализ

Секция для математика / DS, который хочет помочь довести подавление до целевого уровня (98%, 100% — идеал). Все числа ниже воспроизводимы аналитически (формулы приведены) и проверены скриптами на этом коде.

> **Исторический контекст.** Анализ написан в эпоху плато ~50% J (single-scale
...
**Интерпретация плато (~50–63% J):** это уровень «доминантная часть энергии погашена с фазовой ошибкой ~10–30° или частотной ~0.1–0.5 Гц». Т.е. оптимизатор находит правильные частоты, но точная доводка по фазе/частоте в узких гребнях ландшафта остаётся главным тормозом.
...
Шаг по частоте **больше не общий** с остальными параметрами. Coordinate descent использует трёхскоростной абсолютный шаг (offsets 1–2) — по одному значению на цикл:

| Цикл       | Параметр               | Шаг в векторе | ≈ Гц         |
| ---------- | ---------------------- | ------------- | ------------ |
| EXPLORATION | `frequencyStepCoarse`  | $10^{-4}$     | **2 Гц**     |
| REFINEMENT  | `frequencyStepRefine`  | $5\cdot10^{-6}$ | **0.1 Гц**  |
...
2. **Сменить параметризацию частоты** (п. 2): лог-шкала (20 Гц–20 кГц ≈ 10 октав; полутоновая/центовая сетка естественна и для музыкального слуха). ~~Отдельные масштабы шага на класс параметра~~ — ✅ **сделано** (трёхскоростной шаг частоты + отдельный шаг фазы).
...
5. **Согласовать объектив** (п. 7): ~~убрать per-window скейл~~ — ✅ сделано (фиксированный масштаб 1); ~~кешировать спектральный анализ таргета~~ — ✅ сделано (`SpectralProfile`, п. 8). Остаётся: добавить wrap-around фазы.
...
#### Артефакты подбора (контракт)

Целевой контракт каждого запуска реверс-синтеза (CLI, sync API,
async job):

1. **JSON параметров для синтезатора** — приоритетный артефакт:
   конфигурация осцилляторов в формате `ArgCreateSynth` (как пресеты
   в `src/presets.ts`), пригодная для повторного синтеза без
   конвертации. В job-пути хранится в записи job и отдаётся через
   `/api/match/jobs/:id/download-params`.
2. **WAV, синтезированный по подобранным параметрам** — обязателен,
   возвращается как и сейчас: быстрая субъективная оценка результата
   (job-путь: `/api/match/jobs/:id/download`).
3. Метаданные запуска: `suppressionPercent` (surrogate J),
   `globalSuppressionPercent` (честный глобальный suppression финального
   WAV), история прогресса, `targetInfo`.
...
| Трёхскоростные шаги       | Частота: 2 Гц (EXPLORATION) → 0.1 Гц (REFINEMENT) → 0.002 Гц (PRECISION); фаза: 1.1° → 0.3° → 0.07°; остальные параметры — по расписанию цикла                  |
| Simulated annealing       | Кандидат, ухудшающий score на Δ п.п., принимается с вероятностью `exp(−Δ/T)`; T=3 охлаждается ×0.99 за итерацию непрерывно, в PRECISION T=0 (чистый greedy)     |
...
| `on` параметр             | Строго бинарный (`0` или `1`); состояния флагов полностью управляются CD (осциллятор с `volume ≤ VOLUME_PRUNE_THRESHOLD` пробует кандидат `on=0`); osc[0] всегда включён |
| Residual-guided relocation | Слабый осциллятор перед выключением переставляется на доминантный пик остатка `target − synth` (Goertzel-скан по амплитуде + DFT-фаза, exclusion активных, лимит 3 попытки/осц, приёмка по windowed-score после greedy-прохода) |
| Оконечная оценка          | `evaluateMultiScaleWindowed` — useful zone auto-detection + 3 масштаба окон (10/50/200ms) + ringing penalty за энергию вне зоны |
| Scale fitting             | После оптимизации — аналитический оптимальный масштаб (проекция на таргет), множитель пишется в `startLevel`, геном пересинтезируется и переоценивается          |
| Финальный прунинг         | Осцилляторы с `startLevel < VOLUME_PRUNE_THRESHOLD` отключаются по той же windowed-метрике, что и CD; откат, если score падает > 0.05 п.п.                       |
| Ранний выход              | Достижение `98%` suppression или окончание всех циклов                                                                                                          |

> **Важно:** оптимизация вычислительно затратна. Каждая оценка параметра пересинтезирует вклад затронутого осциллятора (инкрементальный `WaveformCache`). Multi-scale evaluation оценивает качество через авто-детекцию полезной зоны, три масштаба окон (10/50/200ms) и ringing penalty за энергию вне зоны.
...
| `src/optimize/`                     | Модуль оптимизации: `index.ts` (реэкспорт), `coordinate-descent.ts` (алгоритм), `evaluate.ts` (multi-scale windowed оценка), `consts.ts` (константы), `types.ts` (типы), `staged.ts` (тонкая обёртка над coordinateDescent), `residual-relocation.ts` (relocation слабых осцилляторов на пики остатка) |
