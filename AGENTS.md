...
- **Прогон детерминирован.** Весь случайный выбор идёт через
  ...
  **Детерминизм действует внутри одной среды, а не между машинами.**
  Замер: один и тот же job (Mark Bester, 300 итераций, seed 1, boost 8,
  один и тот же образ-поколение) дал EXPLORATION до итерации 181 и
  global 89.83% в локальном ARM-контейнере против итерации 204 и 92.53%
  на удалённом сервере. `Math` на ARM-Mac и ARM-Linux совпадает побитово,
  так что причина — различия вычислений с плавающей точкой между
  платформами, которые на хаотичном 500-мерном спуске расходятся в разные
  локальные оптимумы. Практическое следствие: **сравнивать одиночные
  прогоны между машинами нельзя**, только внутри одной среды
...
- **Вес атаки (`attackWeighting` в `evaluate.ts`, параметр job `attackBoost`).**
  ...
  **Оптимальное значение зависит от материала — единого нет.** Замер
  (250 итераций, seed 1, текущий код, одна машина), global при
  `boost=1` → `boost=4`: Boris 90.24% → 90.90% (+0.66), Untitled
  84.08% → 84.60% (+0.52), **Mark Bester 92.81% → 90.15% (−2.66)**.
  Развёртка на Boris: `boost` 1/2/4/8 → 90.24 / 90.17 / **90.90** /
  88.99 — максимум в середине, на восьмёрке рушится низ (остаток
  20–200 Гц 1.0% → 4.5%), а там вся энергия, отсюда и падение global.
  Формулировка «размен качества на число» (была здесь раньше) **неверна**:
  на части таргетов умеренный boost улучшает и global, и атаку, и верхние
  полосы одновременно; на других — проигрывает. Поэтому дефолт `1`
  (нейтральное значение), а подбор отдан в точку использования.
  Ориентир: имеет смысл поднимать там, где остаток в верхах катастрофичен
  (у Boris 82.6% в полосе 1–4 кГц при `boost=1`), и не трогать там, где
  таргет и так подогнан хорошо (Mark Bester: 3.3%).
