...
- **Вес атаки (`attackWeighting` в `evaluate.ts`, параметр job `attackBoost`).**
  ...
  **Цена.** Замер (250 итераций, seed 1, Boris и Mark Bester): `boost=8`
  стоит −1.25 п.п. global на обоих таргетах, но улучшает атаку
  (Boris 41.9% → 21.6%) и верх 1–4 кГц (82.6% → 53.0%). Это размен
  слышимого качества на формальное число, поэтому дефолт выключен, а выбор
  отдан в точку использования.
